...
def get_child_products_from_parent(cursor, parent_product: dict):
    query = (
        f'SELECT gg_posts.ID, gg_posts.post_title, gg_posts.post_name FROM gg_posts '
        f'WHERE post_parent = {parent_product.get("product_id")} '
...
def get_product_size(product: dict):
    ...
    size_raw = product.get("post_name")
    result = re.search(r"\d-rozmir", size_raw)
    size = result.group(0)
    return size[:-7]
